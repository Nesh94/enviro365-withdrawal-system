[README.md](https://github.com/user-attachments/files/32184004/README.md)
...
The package is `com.enviro.assessment.junior.mutshutshudzi` throughout — the
`yourname` placeholder used during development has already been renamed
and pushed.
...
```
src/main/java/com/enviro/assessment/junior/mutshutshudzi/
...
`http://localhost:8080/h2-console` (JDBC URL `jdbc:h2:mem:enviro365db`,
...
> **Verified:** builds and runs cleanly with `mvn spring-boot:run` and
> passes `mvn test`.
...
is running. It calls the API at `http://localhost:8080/api`, configurable
